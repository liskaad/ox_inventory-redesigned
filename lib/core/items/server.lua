...
local Items = {}
local ItemList = require 'lib.core.items.shared' --[[@as table<string, OxServerItem>]]
local Utils = require 'lib.utils.server'

TriggerEvent('ox_inventory:itemList', ItemList)

Items.containers = require 'lib.core.items.containers'
...
CreateThread(function()
	Inventory = require 'lib.core.inventory.server'
	...
	if shared.framework == 'esx' then
		...
		if success and items and next(items) then
			...
			if table.type(dump) ~= "empty" then
				local file = {string.strtrim(LoadResourceFile(shared.resource, 'config/items.lua'))}
				file[1] = file[1]:gsub('}$', '')
	...
	[%q] = {
		...
		description = %q
				...
				SaveResourceFile(shared.resource, 'config/items.lua', table.concat(file), -1)
...
local TriggerEventHooks = require 'lib.core.hooks.server'
